    ...
    pub async fn new(cmd_rx: mpsc::Receiver<NotifierItemCommand>) -> Result<StatusNotifierWatcher> {
...

        {
            ...
            tokio::spawn(async move {
                ...
                if let Err(e) = start_notifier_watcher(tx).await {
                    tracing::error!("StatusNotifierError: {e}");
                }
            });
        }
        ...
        {
            tokio::spawn(async move {
                if let Err(e) = dispatch_ui_command(cmd_rx).await {
                    tracing::error!("UI command dispatch error: {e}");
                }
            });
        }
...
async fn start_notifier_watcher(sender: broadcast::Sender<NotifierItemMessage>) -> Result<()> {
    ...
    let status_notifier = {
        let connection = connection.clone();
        tokio::spawn(async move {
            if let Err(e) = status_notifier_handle(connection, sender).await {
                tracing::error!("StatusNotifier handle error: {e}");
            }
        })
    };
...
async fn fetch_properties_and_update(
    ...
    let interface = match InterfaceName::from_static_str("org.kde.StatusNotifierItem") {
        Ok(i) => i,
        Err(e) => {
            tracing::warn!("[tray] Invalid interface name: {e}");
            return Ok(());
        }
    };
    let props = match dbus_properties_proxy.get_all(interface).await {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!("[tray] Failed to get properties for {item_address}: {e}");
            return Ok(());
        }
    };
...
async fn watch_menu(
    ...
    let menu: MenuLayout = match dbus_menu_proxy.get_layout(0, 10, &[]).await {
        Ok(m) => m,
        Err(e) => {
            tracing::warn!("[tray] Failed to get menu layout: {e}");
            return Err(e.into());
        }
    };
    ...
    tokio::spawn(async move {
        ...
        while props_changed.next().await.is_some() {
            let menu: MenuLayout = match dbus_menu_proxy.get_layout(0, 10, &[]).await {
                Ok(m) => m,
                Err(e) => {
                    tracing::warn!("[tray] Failed to get menu layout update: {e}");
                    continue;
                }
            };
