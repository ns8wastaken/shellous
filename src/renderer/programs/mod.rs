...
/// is as simple as implementing `RenderProgram` and calling
/// `register` — no match arms, no batch changes.
pub struct ProgramRegistry {
    programs: HashMap<Shape, Box<dyn RenderProgram>>,
}
...
impl ProgramRegistry {
    ...
    pub fn register<S: RenderProgram + 'static>(&mut self, shape: Shape, program: S) {
        self.programs.insert(shape, Box::new(program));
    }

    pub fn get(&self, shape: &Shape) -> Option<&dyn RenderProgram> {
